...
router.post('/Addteacherdata', function (req, res, next) {
    ...
    db.query('select * from t_teacher', [], function (result, fields) {
		let tno=1
		if(result.length>0){
			tno = result[result.length - 1].tno + 1
		}
		let idname=body.username
		let searchbyidsql = 'SELECT * FROM t_teacher\n' +
            'WHERE t_teacher.loginname=?\n'
        let searchbyidParams = [idname]
		db.query(searchbyidsql, searchbyidParams, function (result, fields) {
			console.log(result)
            if (result.length>0) {
                console.log('该用户已经重复注册')
                return res.status(200).json({
					result: 'error'
				})
            } else {
...
router.post('/AddteacherAndCourse', function (req, res, next) {
    ...
    db.query('select * from t_course', [], function (result, fields) {
		let cno = 1
		if(result.length>0){
			cno = result[result.length - 1].cno + 1
		}
...
router.get('/StudentAndClass', function (req, res, next) {
	...
	let selectsql = 'select * from  t_student s,t_class c WHERE s.classno = c.cname'
    db.query(selectsql, [], function (result, fields) {
        return res.status(200).json({
            err_code: 0,
            message: 'OK',
...
router.post('/AddStudentAndClass', function (req, res, next) {
    ...
    db.query('select * from t_student', [], function (result, fields) {
        ...
        db.query(findClasstsql, findClassSqlParams, (result) => {
            ...
            let addStudentSqlParams = [sno, PwdOfstudent, NameOfstudent, ContactMethod, gender, DateOfBirth, ClassAndGrade, desc, mail];
...
router.get('/TeacherFindCourse', function (req, res, next) {
    ...
    TeacherFindCoursesql = 'SELECT * FROM t_teacher ,t_course \n' +
